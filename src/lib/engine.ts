...
class GlickoConfig {
    ...
    calculateExpectedScore(scaledRating: number, opponentScaledRating: number, opponentScaledDeviation: number): number {
        const impactFactor = this.calculateDeviationImpactFactor(opponentScaledDeviation);
        return 1 / (1 + Math.exp(-impactFactor * (scaledRating - opponentScaledRating)));
    }
...
class GlickoRating {
    ...
    constructor(config: GlickoConfig, rating = config.initialRating, deviation = config.initialRatingDeviation, volatility = config.initialVolatility) {
        this.config = config;
        this.rating = rating;
    ...
    decay(date: Date): void {
        if (this.lastRankedAt) {
            const periodsElapsed = Math.max(
                0,
                (date.getTime() - this.lastRankedAt.getTime()) / this.config.decayPeriodMs
            );
    ...
    update(opponentRating: GlickoRating, score: number): void {
        ...
        this.deviation = Math.max(config.minRatingDeviation, newPhi * config.inverseScaleFactor);
        this.volatility = newSigma;
    }

    copy(): GlickoRating {
        return new GlickoRating(this.config, this.rating, this.deviation, this.volatility);
    }
}
...
class RunningStat {
    ...
    push(value: number): void {
        ...
        if (this.#n >= MIN_ATTEMPTS_FOR_ANOMALY_DETECTION) {
            const anomalousThreshold = OUTLIER_Z_SCORE_THRESHOLD * this.standardDeviation;
...
abstract class Entity {
    ...
    get compositeRating(): number {
        return this.accuracyRating.rating * COMPOSITE_ACCURACY_WEIGHT + this.speedRating.rating * COMPOSITE_SPEED_WEIGHT;
    }
}

class Human extends Entity {
    constructor() {
        super(HUMAN_ACCURACY_GLICKO_CONFIG.createRating(), HUMAN_SPEED_GLICKO_CONFIG.createRating());
    }
...
abstract class Question<Answer> extends Entity {
    ...
    private readonly continuousMinStandardDeviation: Record<QuestionPartId, number>;

    /** Map from the ID of a discrete part of the question to the number of times humans have answered the part correctly. */
    private readonly discretePartCounts: Record<QuestionPartId, { attempts: number, successes: number }> = {};
    ...
    private readonly speedStats = new RunningStat(MIN_SPEED_STANDARD_DEVIATION_MS);

    constructor(
        discretePartGuessChances: Record<QuestionPartId, number | undefined> = {},
        continuousMinStandardDeviation: Record<QuestionPartId, number | undefined> = {}
    ) {
        super(QUESTION_ACCURACY_GLICKO_CONFIG.createRating(), QUESTION_SPEED_GLICKO_CONFIG.createRating());

        function filterUndefinedValues<T>(obj: Record<string, T | undefined>): Record<string, T> {
        ...
        this.discretePartGuessChances = filterUndefinedValues(discretePartGuessChances);
        this.continuousMinStandardDeviation = filterUndefinedValues(continuousMinStandardDeviation);
    }
    ...
    tuneDifficultyModel(correctParts: Record<QuestionPartId, boolean>, partErrors: Record<QuestionPartId, number>): void {
        ...
        for (const [partId, error] of Object.entries(partErrors)) {
            if (!this.continuousErrorStats[partId]) {
                this.continuousErrorStats[partId] = new RunningStat(this.continuousMinStandardDeviation[partId] ?? 1);
            }
    ...
    calculateAccuracyFromParts(correctParts: Record<QuestionPartId, boolean> = {}, partErrors: Record<QuestionPartId, number> = {}): number {
        ...
        for (const [partId, correct] of Object.entries(correctParts)) {
            ...
            discreteGuessableScore += weight * (this.discretePartGuessChances[partId] ?? 0);
        }
        const adjustedDiscreteScore = discreteGuessableScore === discreteMaxScore
            ? 0
            : Math.max(0, ((discreteScore - discreteGuessableScore) / (discreteMaxScore - discreteGuessableScore)) * discreteMaxScore);
    ...
    calculatePureSpeedScore(answerTimeMs: number): number {
        if (!this.hasSpeedStats()) throw new Error("Cannot calculate speed score for a question with no speed statistics.");
        const zScore = (answerTimeMs - this.speedStats.mean) / this.speedStats.standardDeviation;
    ...
    getDiscreteParts(_correctAnswer: Answer, _givenAnswer: Answer): Record<QuestionPartId, boolean> {
        return {};
    }

    getContinuousParts(_correctAnswer: Answer, _givenAnswer: Answer): Record<QuestionPartId, number> {
        return {};
    }
...
class MultipleChoiceQuestion<Choice extends Primitive> extends Question<Choice> {
    constructor(numOptions: number) {
        super({ "": 1 / numOptions });
    }

    getDiscreteParts(correctAnswer: Choice, givenAnswer: Choice): Record<QuestionPartId, boolean> {
...
class DateQuestion extends Question<Temporal.PlainDate> {
    constructor(options: {
        ...
        super(
            {
                "year": options.numPossibleYears ? 1 / options.numPossibleYears : undefined,
                "month": 1 / 12,
                "day": 1 / 31
            },
    ...
    getDiscreteParts(correctAnswer: Temporal.PlainDate, givenAnswer: Temporal.PlainDate): Record<QuestionPartId, boolean> {
        return {
    ...
    getContinuousParts(correctAnswer: Temporal.PlainDate, givenAnswer: Temporal.PlainDate): Record<QuestionPartId, number> {
...
function updateGlickoRating(playerRating: GlickoRating, opponentRating: GlickoRating, score: number, date: Date): void {
...
function processAnswer<Answer>(human: Human, question: Question<Answer>, correctAnswer: Answer, givenAnswer: Answer, playedAt: Date, answerTimeMs: number): void {
    const correctDiscreteParts = question.getDiscreteParts(correctAnswer, givenAnswer);
    const continuousPartErrors = question.getContinuousParts(correctAnswer, givenAnswer);
    const accuracy = question.calculateAccuracyFromParts(correctDiscreteParts, continuousPartErrors);
    updateGlickoRating(human.proficiencyRating, question.difficultyRating, accuracy, playedAt);
    ...
    if (question.hasSpeedStats()) {
        ...
        updateGlickoRating(human.fluencyRating, question.timeDemandRating, speed, playedAt);
    }
